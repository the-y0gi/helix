...
// Get all hotels Supports check in checkout, filters, city, featured, rating
  ...
  try {
    ...
    if (startDate && endDate) {
      const pipeline = [
        ...
        { $sort: { isFeatured: -1, rating: -1, startingPrice: 1 } },
        { $skip: skip },
        { $limit: limitNum },
      ];
    ...
    const [hotels, total] = await Promise.all([
      Hotel.find(hotelFilter)
        ...
        .select(
          "name city rating numReviews isFeatured images description amenities",
        )
