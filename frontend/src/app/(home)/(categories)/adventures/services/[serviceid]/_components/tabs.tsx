...
import { Calendar, User, ChevronRight, Loader2, Tent } from "lucide-react";
...
type TabKey = "overview" | "description" | "amenities";
    ...
    description: (
      <Decription
        data={{
          ...
          description: "adventure service description from hardcoded ",
        }}
      />
    ),
      ...
      <div className="flex flex-col lg:flex-row lg:gap-6 mb-5">
        ...
        <aside className="lg:w-[380px] flex-shrink-0">
          <div className="lg:sticky lg:top-24 pt-3">
            <AdventureBookingCard data={data} />
          </div>
        </aside>
      </div>
...
// ─── Adventure Booking Sidebar Card ──────────────────────────────────────────
const AdventureBookingCard = ({ data }: { data?: TrekDetailData }) => {
    ...
    <Card className="w-full lg:max-w-[400px] border-none shadow-2xl rounded-[32px] overflow-hidden bg-white/90 dark:bg-zinc-800/90 backdrop-blur-sm p-1 transition-colors duration-300">
      <CardContent className="pt-5 px-5 space-y-5">
        <div className="flex justify-between items-center">
          <div className="space-y-0.5">
            ...
            <div className="flex items-center gap-1.5 text-xs md:text-sm font-semibold text-slate-500 dark:text-slate-400">
              ...
              <span>
                {(data?.adventure as any)?.difficulty || "Moderate"} •{" "}
                {(data?.adventure as any)?.duration || "1 Day"}
              </span>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50/80 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[11px] font-bold text-slate-900 dark:text-slate-200">
                  {date?.from ? format(date.from, "dd/MM/yyyy") : "Select"}
                </span>
              </div>
            </div>
        ...
        {data?.service?.pricing && (
          <div className="bg-gradient-to-r from-primary-50 to-teal-50 dark:from-slate-800 dark:to-slate-800/50 p-4 rounded-2xl border border-primary-100/50 dark:border-slate-700">
            <div className="flex justify-between items-end">
              <div>
                <p className="text-[10px] font-bold text-primary-400 dark:text-primary-500 uppercase tracking-widest">
                  Price
                </p>
                <p className="text-xl font-black text-slate-900 dark:text-slate-50">
                  ₹{data.service.pricing.finalPrice}
      ...
      <CardFooter className="p-5 pt-0">
        <Button
          ...
          onClick={() => {
            ...
            RouterPush(
              router,
              `/booknow/${data.service._id}/${data.adventure._id}`,
              {
          ...
          className="w-full bg-primary hover:bg-primary/80 text-white text-md font-bold h-14 rounded-2xl shadow-lg group active:scale-[0.98] transition-all"
        >
