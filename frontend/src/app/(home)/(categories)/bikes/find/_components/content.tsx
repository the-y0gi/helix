...
import { BikeCard, BikeCardSkeleton } from "./CardCompo";
...
import { useBikeContext } from "@/context/BikesContextProvider";
...
export const ContentFrame = (props: Props) => {
  ...
  const { total, isLoading, page, setPage } = useBikeContext()
  const { city, setCity } = useBikesStore();
  ...
  if (!isLoading && bikes.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 text-muted-foreground">
        <p className="text-lg">No bikes found for the selected filters.</p>
