...
import { CabsCard, CabsCardSkeleton } from "./CardCompo";
import { cn } from "@/lib/utils";
import { Button } from "@/components/ui/button";
import { IconGrid4x4, IconMenu4 } from "@tabler/icons-react";
import { useIsMobile } from "@/hooks/use-mobile";
import { CabService, Hotel } from "@/types";
// import { SheetNavigation } from "./sheetNavigation";
import { ChevronRight } from "lucide-react";
import { useCabsContext } from "@/context/CabsContextProvider";
...
export const ContentFrame = ({ tg }: { tg: string }) => {
  const ismobile = useIsMobile()
  const { total, isLoading, page, setPage } = useCabsContext()
        ...
        <div className="flex gap-3 w-full">
          ...
          <div className="w-full">
            <h2 className="text-2xl font-semibold mb-4 ">
              {total > 0 ? `Explore ${total}+ ${tg}` : `Explore ${tg}`}
            </h2>
...
export const Content = ({ className }: { className: string }) => {
  const { wrap } = useCabsStore();
  const isMobile = useIsMobile()
  const { cabs, isLoading } = useCabsContext()
  ...
  if (!isLoading && cabs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-3 text-muted-foreground">
        <p className="text-lg">No cabs found for the selected filters.</p>
